...
- ?? `services/plan-view.ts` ?? Plan ?????? selector/view-model ????? `DailyTaskView`?`PlanProgress`?`PlanBundleCalendarView`?`PlanCalendarDay`?`TodayView` ???????
- `models/plan.ts` ?????????normalize?`dailyPlansToPlanBundle()` / `planBundleToDailyPlans()` legacy adapter??????????????
- today?calendar?profile?review ?????? storage + service/selector ?????????????? `Plan + Stage + Task` ?????
- `PlanBundle` ??????storage key?DailyPlan legacy adapter ? replanner ?????????
