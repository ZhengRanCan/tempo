...
- Harness ???F12?F13?F14?F15?F16?F17?F18?F19 ?? `passing`?
...
## ??????
- `npm.cmd run test -- plan-calendar today-suggestion today review ui-components data-layer`????6 ??????37 ??????
- `npm.cmd run verify:static`????lint ? vue-tsc --noEmit ???
- `npm.cmd run verify:system`????`build:mp-weixin` ?????
- `npm.cmd run check`????15 ??????93 ??????F19 active ??? harness gate 0 warning / 0 error?
- `npm.cmd run verify:harness`????F19 passing ??? 8 ? feature ?? passing?0 warning / 0 error?

## ????
- `2026-05-27` F19 ??? v0.3 ?????Plan ?????????
- F19 ?? `services/plan-view.ts` ?????? selector/view-model ????? `DailyTaskView`?`PlanProgress`?`PlanBundleCalendarView`?`PlanCalendarDay`?`TodayView` ????????
- F19 ? `models/plan.ts` ?????????normalize ? `dailyPlansToPlanBundle()` / `planBundleToDailyPlans()` legacy adapter??????????????
- today?calendar?profile?review ?????? storage + service/selector ???????????? PlanBundle ??????storage key?DailyPlan legacy adapter ? replanner ?????
