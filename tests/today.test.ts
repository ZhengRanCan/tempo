...
describe('today view', () => {
  ...
  it('keeps the today page focused on one execution task before status, advice and review', () => {
    ...
    const allTaskPanelIndex = source.indexOf('class="all-task-panel"')
    ...
    expect(allTaskPanelIndex).toBeGreaterThan(focusIndex)
    expect(statusIndex).toBeGreaterThan(allTaskPanelIndex)
    expect(adviceIndex).toBeGreaterThan(statusIndex)
    expect(reviewIndex).toBeGreaterThan(adviceIndex)
    expect(source).toContain(':task-count="taskCount"')
    expect(source).toContain('@toggle-all="toggleAllTasks"')
    expect(source).toContain('v-if="showAllTasks"')
    expect(source).toContain('今日全部任务')
    expect(source).toContain('重点已置顶')
  })

  it('matches the F25 execution desk structure without taking calendar duties', () => {
    const source = readProjectFile('pages/today/index.vue')

    expect(source).toContain('@start="startFocus"')
    expect(source).toContain('@done="markFocusDone"')
    expect(source).toContain('调整今日状态')
    expect(source).toContain('/static/icons/page/today/status-wave.png')
    expect(source).toContain('energyOptions')
    expect(source).toContain("label: '更多'")
    expect(source).toContain('timeOptions')
    expect(source).toContain("label: '自定义'")
    expect(source).toContain('moodOptions')
    expect(source).toContain("label: '有点焦虑'")
    expect(source).toContain("label: '想冲刺'")
    expect(source).toContain('visibleSuggestionTips')
    expect(source).toContain('/static/icons/page/today/sparkle.png')
    expect(source).toContain('/static/icons/page/today/moon.png')
    expect(source).not.toContain('todayIconPaths')
    expect(source).not.toContain(':src="')
    expect(source).toContain('goReview')
    expect(source).toContain('晚上记录完成情况，我会自动帮你重排明天。')
    expect(source).not.toContain('savePlanBundle')
    expect(source).not.toContain('saveDailyPlans')
  })
  ...
  it('reads through TodaySuggestion and PlanBundle services without taking calendar duties', () => {
    ...
    expect(source).toContain('/pages/plan-calendar/index')
  })
})
